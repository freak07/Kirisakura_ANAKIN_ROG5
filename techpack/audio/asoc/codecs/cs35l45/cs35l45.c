...
//#define MDSYNC_CS
#define MASK_UNUSED_IRQ
#define LDPM_CONFIG_WORKAROUND
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	unsigned int sts, i;
	int ret;
	#if defined(LDPM_CONFIG_WORKAROUND)
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		switch(dsp->fw) {
			...
			case 10:
				regmap_read(cs35l45->regmap, CS35L45_LDPM_CONFIG, &cs35l45->ldpm_config);
				regmap_write(cs35l45->regmap, CS35L45_LDPM_CONFIG, 0x1B636);
				break;
			//case WM_ADSP_FW_SPK_PROT:
			case 9:
				regmap_write(cs35l45->regmap, CS35L45_LDPM_CONFIG, cs35l45->ldpm_config);
				break;
			default:
				break;
		}
				 ...
				 struct snd_soc_dai *dai)
{
	...
	/* To support that using same RX/TX TDM format without independent capture dai link */
	//} else {
		regmap_update_bits(cs35l45->regmap, CS35L45_ASP_CONTROL2,
...
static int cs35l45_global_error_release(struct cs35l45_private *cs35l45)
{
	...
	dev_dbg(cs35l45->dev, "%s: count = %u\n", __func__, cs35l45->global_err_rls);
...
static const struct cs35l45_irq_monitor cs35l45_irq_mons[] = {
	...
	},
	{
		...
		.callback = cs35l45_global_error_release,
	},
	{
		.reg = CS35L45_IRQ1_EINT_1,
		.mask = CS35L45_IRQ1_MASK_1,
	...
	},
	{
		.reg = CS35L45_IRQ1_EINT_1,
		.mask = CS35L45_IRQ1_MASK_1,
		.bitmask = CS35L45_BST_UVP_ERR_MASK,
		.description = "Boost under voltage error",
		.err_msg = "Boost under voltage error detected!",
		.callback = NULL,
		//.callback = cs35l45_global_error_release,
	},
	{
		.reg = CS35L45_IRQ1_EINT_1,
		.mask = CS35L45_IRQ1_MASK_1,
		.bitmask = CS35L45_TEMP_ERR_MASK,
		.description = "Over temperature error",
		.err_msg = "Over temperature error detected!",
		.callback = NULL,
		//.callback = cs35l45_global_error_release,
	},
	{
		.reg = CS35L45_IRQ1_EINT_1,
		.mask = CS35L45_IRQ1_MASK_1,
		.bitmask = CS35L45_UVLO_VDDBAT_ERR_MASK,
		.description = "UVLO VDDBAT error",
		.err_msg = "UVLO VDDBAT error detected!",
		.callback = NULL,
		//.callback = cs35l45_global_error_release,
	},
...
#endif
	{
		...
		.bitmask = CS35L45_UVLO_VDDLV_ERR_MASK,
		.description = "UVLO VDDLV error",
		.err_msg = "UVLO VDDLV error detected!",
		.callback = NULL,
		//.callback = cs35l45_global_error_release,
	},
	{
...
static irqreturn_t cs35l45_irq(int irq, void *data)
{
	...
	dev_err(cs35l45->dev, "%s: enter\n", __func__); /* ASUS_BSP Paul +++ */
...
#if defined(MASK_UNUSED_IRQ)
	for (i = 0; i < ARRAY_SIZE(irq_masks); i++) {
		regmap_write(cs35l45->regmap, irq_masks[i], 0xffffffff);
	}
#endif
...
static int __cs35l45_initialize(struct cs35l45_private *cs35l45)
{
	...
	for (i = 0; i < 5; i++) {
		...
		cs35l45->otp_boot_done = true;

		break;
	}

	if (i == 5 && !cs35l45->otp_boot_done) {
