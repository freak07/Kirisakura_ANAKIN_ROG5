...
bool dp_asus_validate_dp_version(struct dp_panel *dp_panel)
{
	if (!dp_panel->asus_vendor)
		return false;

	// some specific monitor report the wrong DP version (DP 1.4 > DP 1.2)
	// to avoid training failed due to select the wrong training pattern
	// PA329C, XG49VQ
	if (!strncmp(dp_panel->asus_vendor, "AUS", 3) &&
			(dp_panel->asus_proc_codes == 0x49a ||
			 dp_panel->asus_proc_codes == 0x326))
		return true;
