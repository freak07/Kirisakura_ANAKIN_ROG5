...
static struct iris_reg _esd_regs[] = {
	{0xf1100000, "MIPI_RX.DPHY_ITF_CTRL"},
	{0xf1100004, "MIPI_RX.DPHY_DSI_ERR_CNT"},
	{0xf110000c, "MIPI_RX.DPHY_DSI_ERR_PROC"},
	{0xf110002c, "MIPI_RX.RX_CTRL_ST_IDLE"},
	{0xf1100034, "MIPI_RX.INTSTAT_RAW"},
	{0xf1100204, "MIPI_RX.DCS_CMD_04"},
	{0xf110040c, "MIPI_RX.DSI_G_WRXP"},
	{0xf1100420, "MIPI_RX.DSI_G_RESP"},
	{0xf1100440, "MIPI_RX.DSI_G_CMDCUR"},
	{0xf1100444, "MIPI_RX.DSI_G_CMDNXT"},
	{0xf1100450, "MIPI_RX.DSI_G_BUF_ST0"},
	{0xf1100454, "MIPI_RX.DSI_G_BUF_ST1"},
	{0xf1101454, "MIPI_RX.RX_CTRL_ST_LPRX_1"},
	{0xf1101460, "MIPI_RX.RX_CTRL_ST_CMD_0"},
	{0xf1101464, "MIPI_RX.RX_CTRL_ST_CMD_1"},
	{0xf1101468, "MIPI_RX.RX_CTRL_ST_CMD_2"},
	{0xf110146c, "MIPI_RX.RX_CTRL_ST_CMD_3"},
	{0xf1101470, "MIPI_RX."},
	{0xf0000038, "SYS.DYNAMIC_PWR_CTRL"},
	{0xf000003c, "SYS.SW_PWR_CTRL"},
	{0xf0000040, "SYS.PMU_CTRL"},
	{0xf0000064, "SYS.TOP_PMU_STATUS"},
	{0xf0000068, "SYS.MIPI_PMU_STATUS"},
	{0xf000006c, "SYS.PQ_PMU_STATUS"},
	{0xf00000d0, "SYS.GLOBAL_STATUS"},
	{0xf000028c, "SYS.AON_STAT_INFO0"},
	{0xf0000290, "SYS.AON_STAT_INFO1"},
	{0xf0010004, "SYS.AON_ABP_CTRL"},
	{0xf0010014, "SYS.MINIPMU_STAT_0"},
	{0xf0010018, "SYS.MINIPMU_STAT_1"},
	{0xf001ffe4, "SYS.INTSTAT_RAW"},
	{0xf1400070, "MIPI_TX.DSI_TX_STATUS"},
	{0xf1400074, "MIPI_TX.DSI_TX_INTERNAL_STATUS"},
	{0xf1400078, "MIPI_TX.DSI_TX_RESERVE_STATUS"},
	{0xf141ffe4, "MIPI_TX.INTSTAT_RAW"},

	{0xf1140000, "PWIL.PWIL_CTRL"},
	{0xf1140004, "PWIL.DATA_PATH_CTRL"},
	{0xf1140008, "PWIL.DATA_PATH_CTRL1"},
	{0xf114000c, "PWIL.DISP_RES"},
	{0xf1140044, "PWIL.CAPT_CTRL0"},
	{0xf1140130, "PWIL.PWIL_STATUS"},
	{0xf1140138, "PWIL.IDLE_STATUS"},
	{0xf115ffe4, "PWIL.INTSTAT_RAW"},
	{0xf10dffe4, "DTG.INTSTAT_RAW"},
	{0xf119ffe4, "DSC_DEC.INTSTAT_RAW"},
	{0xf11c0690, "HDR.HDR_PB_STATUS"},
	{0xf11dffe4, "HDR.INTSTAT_RAW"},
	{0xf125ffe4, "SCALER.INTSTAT_RAW"},
	{0xf129ffe4, "PB_SELECT.INTSTAT_RAW"},
	{0xf131ffe4, "DBC.INTSTAT_RAW"},
	{0xf1380000, "DPORT.DPORT_CTRL0"},
	{0xf1380004, "DPORT.DPORT_CTRL2"},
	{0xf1380008, "DPORT.DPORT_CTRL1"},
	{0xf1380048, "DPORT.PB_BUS_STATUS"},
	{0xf1380058, "DPORT.DPORT_STATUS"},
	{0xf139ffe4, "DPORT.INTSTAT_RAW"},
	{0xf13dffe4, "DSC_ENC.INTSTAT_RAW"},
	{0xf148044c, "DPP.PB_BUS_STATUS"},
	{0xf149ffe4, "DPP.INTSTAT_RAW"},
	{0xf161ffe4, "LUT_WRAP.INTSTAT_RAW"},
};
...
exit_abyp_loop:
	...
	if (abyp_status_gpio != 0 || rc == -2) {
		...
		//iris_dump_regs(_esd_regs, ARRAY_SIZE(_esd_regs)); // do not dump log since this WA has no side effect
		iris_reset();
		iris_lightup_exit_abyp(true, true);
		iris_preload();
		goto exit_abyp_loop;
...
static void iris_dump_hdr_reg(void)
{
	int i = 0, ret = 0;
	u32 write_val, read_val;
	u32 regs[2] = {0xf11c0684, 0xf11c0688};

	for (i = 0; i < 32; i++) {
		write_val = 1 << i;
		ret = iris_ioctl_i2c_write(regs[0], write_val);
		if (ret) {
			IRIS_LOGE("%s(%d), i2c fails to write 0x%x,val:%d", __func__, __LINE__, regs[0], i);
			return;
		}

		ret = iris_ioctl_i2c_read(regs[1], &read_val);
		if (ret) {
			IRIS_LOGE("%s(%d), i2c fails to read 0x%x, ret:%d", __func__, __LINE__, regs[1], ret);
		} else {
			char name[32];
			snprintf(name, 32, "%s [%s %08x]", "HDR.DEBUG_BUS", "MUX", write_val);
			IRIS_LOGE("[%02d %-32s] %08x : %08x", i, name, regs[0], read_val);
		}
	}
}

void iris_dump_regs(struct iris_reg *regs, u32 len)
{
	u32 value, i;
	struct iris_cfg *pcfg;
	...
	for (i = 0; i < len; i++) {
		if (pcfg->read_path == PATH_I2C) //use i2c to read
			iris_ioctl_i2c_read(regs[i].addr, &value);
		else {
			value = iris_ocp_read(regs[i].addr, DSI_CMD_SET_STATE_HS);
		}
		IRIS_LOGE("[%02d %-32s] %08x : %08x", i, regs[i].name, regs[i].addr, value);
	}
