...
int iris_reset(void)
{
	int rc = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
	struct dsi_panel_reset_config *r_config = &pcfg->panel->reset_config;
	int reset_gpio = r_config->px_reset_gpio;

	if (gpio_is_valid(reset_gpio)) {
		rc = gpio_direction_output(reset_gpio, IRIS_GPIO_LOW);
		if (rc != 0) {
	...
	gpio_set_value(reset_gpio, IRIS_GPIO_LOW);
	usleep_range(2000, 2001);
	gpio_set_value(reset_gpio, IRIS_GPIO_HIGH);
