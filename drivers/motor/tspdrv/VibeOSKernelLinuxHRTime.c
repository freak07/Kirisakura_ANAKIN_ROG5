...
#define TSPDRV_DIAGNOSTICS
#ifdef TSPDRV_DIAGNOSTICS
static ktime_t processing = 0;
#endif

#ifndef NUM_EXTRA_BUFFERS
//#define NUM_EXTRA_BUFFERS 0
#define NUM_EXTRA_BUFFERS 30
#endif
...
static void VibeOSKernelLinuxInitTimer(void)
{
    ...
    //wakeup_source_init(&g_tspWakelock, "tspdrv");    Deprecated in kenel 5.4
...
#ifdef TSPDRV_DIAGNOSTICS
    processing = ktime_to_us(ktime_get());
#endif
...
{
#ifdef TSPDRV_DIAGNOSTICS
    /* diagnostic log from main thread, not timer */
    if (!hrtimer_callback_running(&g_tspTimer))
    {
        /* check if down_interruptible is waiting forever */
        if (!hrtimer_active(&g_tspTimer) && VibeSemIsLocked(&g_hSemaphore))
        {
            s64 remaining = ktime_to_us(hrtimer_get_remaining(&g_tspTimer));
            DbgOutErr(("VibeOSKernelLinuxStopTimer: locked but timer inactive (remaining=%ldus watchdog=%d, started=%d)",
                remaining, g_nWatchdogCounter, g_bTimerStarted));
        }

        /* check if VibeOSKernelProcessData is hung */
        if (processing)
        {
            s64 elapsed = ktime_us_delta(ktime_get(), processing);
            DbgOutErr(("VibeOSKernelLinuxStopTimer: called stop while processing data started %ldus ago", elapsed));
        }
    }
#endif
...
static void VibeOSKernelLinuxTerminateTimer(void)
{
    ...
    //wakeup_source_trash(&g_tspWakelock);  Deprecated in kenel 5.4
