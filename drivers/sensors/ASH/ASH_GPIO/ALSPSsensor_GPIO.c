...
#ifdef BACK_PSENSOR
#define GPIO_BACK_P_LOOKUP_STATE	"alsps_gpio_p_no_pull"
#endif
...
#ifdef BACK_PSENSOR
	set_state = pinctrl_lookup_state(key_pinctrl, GPIO_BACK_P_LOOKUP_STATE);
	ret = pinctrl_select_state(key_pinctrl, set_state);
	if(ret < 0)
		err("%s: pinctrl_select_state back_p ERROR(%d).\n", __FUNCTION__, ret);
#endif
}
#endif
