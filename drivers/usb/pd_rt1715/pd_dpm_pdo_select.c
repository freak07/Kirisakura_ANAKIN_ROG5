...
{
	/* TODO */
	DPM_DBG("[Bottom_PD] dpm_select_pdo_from_custom\n");
	...
	struct dpm_pdo_info_t *sink, struct dpm_pdo_info_t *source)
{
	...
	DPM_DBG("[Bottom_PD] dpm_select_pdo_from_max_power\n");
#ifdef CONFIG_USB_PD_ALT_MODE_RTDC
		...
		struct dpm_pdo_info_t *sink, struct dpm_pdo_info_t *source)
{
	...
	DPM_DBG("[Bottom_PD] dpm_select_pdo_from_pps\n");

	DPM_DBG("select_pdo sink_type=%d, source_type=%d\n", sink->type, source->type);
	if (sink->type != DPM_PDO_TYPE_APDO || source->type != DPM_PDO_TYPE_APDO)
		return false;

	DPM_DBG("select_pdo apdo_type=%d\n", source->apdo_type);
	if (!(source->apdo_type & DPM_APDO_TYPE_PPS))
		return false;

	DPM_DBG("select_pdo sind_vmax=%d, source_vmax=%d\n", sink->vmax, source->vmax);
	if (sink->vmax > source->vmax)
		return false;

	DPM_DBG("select_pdo sind_vmin=%d, source_vmin%d\n", sink->vmin, source->vmin);
	if (sink->vmin < source->vmin)
		return false;
	...
	DPM_DBG("select_pdo uw=%d, diff_mv=%d\n", uw, diff_mv);
	...
	DPM_DBG("select_pdo, max_uw=%d, cur_mv=%d\n", select_info->max_uw, select_info->cur_mv);
	if (overload) {
		select_info->max_uw = uw;
	...
	DPM_DBG("select_pdo, return_ false\n");
	return false;
}
#endif	/* CONFIG_USB_PD_REV30_PPS_SINK */
