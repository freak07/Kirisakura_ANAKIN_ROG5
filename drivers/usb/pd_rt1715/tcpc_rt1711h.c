...
printk("%s: Set rt1711 pinctrl state over\n", __func__);
	...
	if (g_ASUS_hwID == HW_REV_ER){
		gpio_ctrl->BTM_OTG_EN = of_get_named_gpio(chip->dev->of_node, "BTM_OTG_EN_ER", 0);
		rc = gpio_request(gpio_ctrl->BTM_OTG_EN, "BTM_OTG_EN_ER");
	}
	else{
		gpio_ctrl->BTM_OTG_EN = of_get_named_gpio(chip->dev->of_node, "BTM_OTG_EN", 0);
		rc = gpio_request(gpio_ctrl->BTM_OTG_EN, "BTM_OTG_EN");
	}
	if (rc)
		pr_info("failed to request BTM_OTG_EN, g_ASUS_hwID = %d\n", g_ASUS_hwID);
	else
		pr_info("Success to request BTM_OTG_EN, g_ASUS_hwID = %d\n", g_ASUS_hwID);
				...
				const struct i2c_device_id *id)
{
	...
	gpio_ctrl = devm_kzalloc(&client->dev, sizeof(*gpio_ctrl), GFP_KERNEL);
	if (!gpio_ctrl)
		return -ENOMEM;
	//ASUS BSP allocate GPIO control ---

	chip->i2c_suspend_flag = false;
	if (use_dt)
