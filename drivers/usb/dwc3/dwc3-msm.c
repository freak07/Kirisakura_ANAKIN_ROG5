...
#if defined ASUS_ZS673KS_PROJECT
static ssize_t mode_POGO_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct dwc3_msm *mdwc = dev_get_drvdata(dev);

	dev_info(mdwc->dev, "[USB] cur_mode_POGO %d, in_host_mode=%d\n", cur_mode_POGO, mdwc->in_host_mode);
	if (cur_mode_POGO==0)
		return scnprintf(buf, PAGE_SIZE, "none\n");
	else if (cur_mode_POGO==1)
		return scnprintf(buf, PAGE_SIZE, "inbox\n");

	return scnprintf(buf, PAGE_SIZE, "0\n");
}
static ssize_t mode_POGO_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct dwc3_msm *mdwc = dev_get_drvdata(dev);

	if (sysfs_streq(buf, "inbox")){
		cur_mode_POGO = 1;
	} else if (sysfs_streq(buf, "none")){
		cur_mode_POGO = 0;
	} else {
		dev_err(mdwc->dev, "[USB] mode_POGO error\n");
		return count;
	}

	if(pre_mode_POGO==cur_mode_POGO) {
...
#if defined ASUS_ZS673KS_PROJECT || defined ASUS_PICASSO_PROJECT
		dev_info(mdwc->dev, "let qit call msm_dwc3_register_switch\n");
#else
		switch (dwc->dr_mode) {
