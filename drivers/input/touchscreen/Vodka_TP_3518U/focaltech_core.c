...
u8 FTS_gesture_register_D1 = 0;
u8 FTS_gesture_register_D2 = 0;
u8 FTS_gesture_register_D5 = 0;
u8 FTS_gesture_register_D6 = 0;
u8 FTS_gesture_register_D7 = 0;
u8 FTS_gesture_register_D8 = 0;

int fts_init_success = 0;
EXPORT_SYMBOL(fts_init_success);
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    FTS_INFO("this is vodka tp driver ! %s", FTS_DRIVER_VERSION);
    ...
    mutex_init(&ts_data->suspend_mutex);
    mutex_init(&ts_data->resume_mutex);
...
*/
    ...
    ts_data->suspend_resume_wq = create_singlethread_workqueue("fts_suspend_resume_wq");
    if (NULL == ts_data->suspend_resume_wq) {
        FTS_ERROR("create resume workqueue failed");
        goto err_create_wq_failed;
    }

    ts_data->read_gesture_wq = create_singlethread_workqueue("fts_read_gesture_wq");
    if (NULL == ts_data->read_gesture_wq) {
        FTS_ERROR("create readgesture workqueue failed");
        goto err_create_wq_failed;
    }

    INIT_WORK(&ts_data->resumework, focal_resume_work);
...
int fts_ts_suspend(void)
{
    ...
    if (fts_init_success == 0) {
        FTS_INFO("[Focal][Touch] Driver does not init successful, skip suspend.");
...
static void focal_resume_work(struct work_struct *work)
{
    ...
    if (fts_init_success == 0) {
        FTS_INFO("[Focal][Touch] Driver does not init successful, skip resume.");
        mutex_unlock(&fts_data->suspend_mutex);
        return ;
    }
...
int fts_ts_resume(void)
{
	if (fts_init_success == 1) {
		queue_work(fts_data->suspend_resume_wq, &fts_data->resumework);
	} else {
		FTS_INFO("[Focal][Touch] Driver does not init successful, skip resume.");
	}
	return 0;
...
{
	printk("[Focal][touch] fts suspend function! \n");
	return 0;
}
