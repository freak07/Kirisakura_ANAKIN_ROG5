...
void focal_usb_detection(bool plugin)
{
	...
	if (plugin) {	/* AC plug in */
		if (!ts_data->charger_mode) {
			FTS_INFO("[%s] enter charger mode", __func__);
			ret = fts_ex_mode_switch(MODE_CHARGER, ENABLE);
			if (ret >= 0) {
				ts_data->charger_mode = ENABLE;
			}
		} else {
			FTS_INFO("[%s] already in charger mode", __func__);
		}
	} else {	/* No AC */
		if (ts_data->charger_mode) {
			FTS_INFO("[%s] exit charger mode", __func__);
			ret = fts_ex_mode_switch(MODE_CHARGER, DISABLE);
			if (ret >= 0) {
				ts_data->charger_mode = DISABLE;
			}
		} else {
			FTS_INFO("[%s] not in charger mode", __func__);
		}
	}
}
