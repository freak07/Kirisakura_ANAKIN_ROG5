...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "fts_ts2"
#define INTERVAL_READ_REG                   200  /* unit:ms */
#define TIMEOUT_READ_REG                    1000 /* unit:ms */
#if FTS_POWER_SOURCE_CUST_EN
#define FTS_VTG_MIN_UV                      2900000
#define FTS_VTG_MAX_UV                      2900000
#define FTS_I2C_VTG_MIN_UV                  1800000
#define FTS_I2C_VTG_MAX_UV                  1800000
#endif

#define Touch_AREA_BASE                     0x01
#define Touch_ID_BASE                       30
/*****************************************************************************
...
static int asus_gesture_mode_resume (struct device *dev);
/*****************************************************************************
...
void fts_ts_suspend_resume(bool suspend) {
    FTS_FUNC_ENTER();
    if (suspend)
...
void asus_gesture_resume(bool resume) {
    FTS_FUNC_ENTER();
    if (resume)
	asus_gesture_mode_resume(fts_data->dev);
    FTS_FUNC_EXIT();
}
...
/*	    FTS_DEBUG("[B]id:%d(x:%d, y:%d, p:%d,area:%d) DOWN!",
                          events[i].id,
                          events[i].x, events[i].y,
                          events[i].p, events[i].area);
*/
	    if (fts_data->report_key) {
		if ((events[i].y > 0 && events[i].y < 1000)) {
		    if (fts_data->ftm){
		      input_report_key(data->input_dev, KEY_B, 1);
		    } else
		    input_report_key(data->input_dev, KEY_R, 1);
		}
    
		if ((events[i].y > 1000 && events[i].y < 2000)) {
		    if (fts_data->ftm){
...
*/
    ...
    if (va_reported) {
	    ...
	    if (fts_data->report_key) {
		if (fts_data->ftm){
		    input_report_key(data->input_dev, KEY_B, 0);
		    input_report_key(data->input_dev, KEY_C, 0);
		} else {
		    input_report_key(data->input_dev, KEY_L, 0);
		    input_report_key(data->input_dev, KEY_R, 0);
		}
	    } else {
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    if (data->asus_gesture_en) {
        if (0 == asus_gesture_readdata(data, NULL)) {
//            FTS_INFO("succuss to get asus gesture data in irq handler");
            return 1;
        }
    }
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < max_touch_num; i++) {
        ...
        events[i].id = (buf[FTS_TOUCH_ID_POS + base] >> 4) + Touch_ID_BASE;
        events[i].area = (buf[FTS_TOUCH_AREA_POS + base] >> 4)+ Touch_AREA_BASE;
...
//    FTS_FUNC_ENTER();
    ...
    __set_bit(KEY_R, input_dev->keybit);
...
static void fts_gesture_resume_work(struct work_struct *work)
{
    struct fts_ts_data *ts_data = container_of(work, struct fts_ts_data,
                                  resume_work);

    asus_gesture_mode_resume(ts_data->dev);
}
#if defined ASUS_ZS673KS_PROJECT  
                                 ...
                                 unsigned long event, void *data)
{
    ...
    blank = evdata->data;
//    FTS_INFO("1499 DRM event:%lu,blank:%d", event, *blank);
    switch (*blank) {
    ...
    case DRM_PANEL_BLANK_POWERDOWN:
        if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
    	    if (fts_data->asus_gesture_en == ENABLE){
      		cancel_work_sync(&fts_data->resume_work);
      		cancel_work_sync(&fts_data->gesture_resume_work);
		fts_ts_suspend(ts_data->dev);
		break;
	    }
...
//    FTS_FUNC_ENTER();
	...
	INIT_WORK(&ts_data->gesture_resume_work, fts_gesture_resume_work);
    }
...
static int asus_gesture_mode_resume (struct device *dev)
{
    ...
    set_vaild_length(ts_data->vaild_len);
...
//    int ret = 0;
    ...
    ts_data->init_workqueue = alloc_ordered_workqueue("fts2_init",0);
    if (!ts_data->init_workqueue) {
