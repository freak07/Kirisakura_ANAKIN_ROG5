...
#define PROC_NAME                               "ftxxxx-debug2"
...
static ssize_t report_keycode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
   
    count = snprintf(buf + count, PAGE_SIZE, "Report key:%s\n",
                     fts_data->report_key ? "On" : "Off");

    return count;
}

static ssize_t report_keycode_store(
    struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!fts_data->report_key) {
            FTS_DEBUG("Report key enable");
            fts_data->report_key = ENABLE;
        }
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (fts_data->report_key) {
            FTS_DEBUG("Report key disable");
            fts_data->report_key = DISABLE;
        }
    }

    FTS_DEBUG("Report key:%d", fts_data->report_key);
    return count;
}

static ssize_t ftm_mode_show (
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
   
    count = snprintf(buf + count, PAGE_SIZE, "FTM mode:%s\n",
                     fts_data->ftm ? "On" : "Off");

    return count;
}

static ssize_t ftm_mode_store(
    struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!fts_data->ftm) {
            FTS_DEBUG("FTM mode enable");
            fts_data->ftm = ENABLE;
        }
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (fts_data->ftm) {
            FTS_DEBUG("FTM mode disable");
            fts_data->ftm = DISABLE;
        }
    }

    FTS_DEBUG("Report key:%d", fts_data->ftm);
    return count;
}

static ssize_t bktp_activate_show (
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
   
    count = snprintf(buf + count, PAGE_SIZE, "%s\n",
                     fts_data->activate ? "Yes" : "No");

    return count;
}

static ssize_t bktp_activate_store(
    struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
// default off
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_DEBUG("Back touch activate");
        fts_data->activate = ENABLE;
	fts_ts_suspend_resume(false);
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_DEBUG("Back touch deactivate");
        fts_data->activate = DISABLE;
	fts_ts_suspend_resume(true);
    }

    FTS_DEBUG("Back touch active :%s", fts_data->activate ? "Yes" : "No");
    return count;
}

static ssize_t bktp_mute_show (
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
   
    count = snprintf(buf + count, PAGE_SIZE, "%s\n",
                     fts_data->mute ? "Yes" : "No");

    return count;
}

static ssize_t bktp_mute_store(
    struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
// default off
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_DEBUG("Back touch mute");
        fts_data->mute = ENABLE;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_DEBUG("Back touch report");
        fts_data->mute = DISABLE;
    }

    FTS_DEBUG("Back touch mute :%s", fts_data->mute ? "Yes" : "No");
    return count;
}
...
static DEVICE_ATTR(report_keycode, S_IRUGO | S_IWUSR, report_keycode_show, report_keycode_store);
static DEVICE_ATTR(ftm_mode, S_IRUGO | S_IWUSR, ftm_mode_show, ftm_mode_store);
static DEVICE_ATTR(bktp_activate, S_IRUGO | S_IWUSR, bktp_activate_show, bktp_activate_store);
static DEVICE_ATTR(bktp_mute, S_IRUGO | S_IWUSR, bktp_mute_show, bktp_mute_store);
/*ASUS add --- */
...
/*ASUS add +++ */    
    ...
    &dev_attr_report_keycode.attr,
    &dev_attr_ftm_mode.attr,
    &dev_attr_bktp_activate.attr,
    &dev_attr_bktp_mute.attr,
/*ASUS add --- */     
    NULL
};
